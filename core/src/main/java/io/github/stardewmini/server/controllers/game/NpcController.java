...
public class NpcController {
    ...
    public static String friendShipNpc(NPC npc,String username) {
        NPCFriendship friendship = getNPCFriendship(npc.getName(),username);
        return  "friendship level : " + friendship.getLevel() + "friendship XP : " + friendship.getXP();
    }

    public static Result meetsNpc(String npcName,String username) {
        ...
        NPCFriendship npcFriendship = getNPCFriendship(npcName,username);
    ...
    public static NPCFriendship getNPCFriendship(String npcName,String username){
        for (NPCFriendship npcFriendship : GameApp.getCurrentGame().getPlayerByUsername(username).getNpcFriendships()){
    ...
    public static Result getNPCsFriendship(String username){
    ...
    public static Result giftNpc(NPC npc, String itemName,String username) {
        if(npc == null){
            return new Result(false,"NPC not found");
        }
        ...
        if(CommonGameController.removeItemFromInventory(item,1,username) == false){
            return new Result(false,"you don't have such item");
        }
        npc.setAnimationTime(0);
        NPCFriendship npcFriendship = getNPCFriendship(npc.getName(), username);
    ...
    public static Result questList(NPC npc,String username) {
        ArrayList<String> list = new ArrayList<>();
        NPCFriendship npcFriendship = NpcController.getNPCFriendship(npc.getName(),username);
    ...
    public static Result questFinish(String number,NPC npc,String username) {
        int i;
        try{
...
//        }
        ...
        if(! CommonGameController.removeItemFromInventory(item, quest.getRequestedItemCount(),username)){
            return new Result(false,"not enough item");
        }

        if (quest.getReward().equals("Coin")){
            GameApp.getCurrentGame().getPlayerByUsername(username).increaseEnergy(quest.getRewardCount());
        }
            ...
            GameApp.getCurrentGame().getPlayerByUsername(username).getLearnedFoodRecipes().add(recipe);
        }
        else{
            Item temp = CommonGameController.findItem(quest.getReward());
            GameApp.getCurrentGame().getPlayerByUsername(username).getBackpack().addItem(temp, quest.getRewardCount());
        }
